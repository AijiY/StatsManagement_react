const apiUrl = process.env.REACT_APP_API_URL; // ローカル開発時はlocalhost、デプロイ時はSecrets参照
...
export const getLeagueRegulationsByLeague = async (
  leagueId,
  setLeagueRegulations
) => {
  try {
...
export const getStanding = async (leagueId, seasonId, setStanding) => {
  try {
    const response = await fetch(
      `${apiUrl}/leagues/${leagueId}/standings/${seasonId}`
    );
    if (!response.ok) {
      const text = await response.text();
      throw new Error(text);
...
export const getPlayerGameStats = async (
  ...
  try {
    const response = await fetch(
      `${apiUrl}/players/${playerId}/player-game-stats/${seasonId}`
    );
    if (!response.ok) {
      const text = await response.text();
      throw new Error(text);
...
export const getPlayersSeasonStatsByClub = async (
  clubId,
  seasonId,
  ...
  try {
    const response = await fetch(
      `${apiUrl}/clubs/${clubId}/players-season-stats/${seasonId}`
    );
    if (!response.ok) {
      const text = await response.text();
      throw new Error(text);
...
export const getPlayerSeasonStats = async (
  ...
  try {
    const response = await fetch(
      `${apiUrl}/players/${playerId}/player-season-stats/${seasonId}`
    );
    if (!response.ok) {
      const text = await response.text();
      throw new Error(text);
...
export const getPlayerCareerStat = async (playerId, setPlayerCareerStat) => {
  try {
    const response = await fetch(
      `${apiUrl}/players/${playerId}/player-career-stat`
    );
    if (!response.ok) {
      const text = await response.text();
      throw new Error(text);
...
export const getSeasonGameReslt = async (
  leagueId,
  seasonId,
  setSeasonGameResult
) => {
  try {
    const response = await fetch(
      `${apiUrl}/leagues/${leagueId}/season-game-results/${seasonId}`
    );
    if (!response.ok) {
      const text = await response.text();
      throw new Error(text);
