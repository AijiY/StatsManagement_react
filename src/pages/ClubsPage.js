...
function ClubsPage() {
  ...
  const [isClubsListView, setIsClubsListView] = useState(
    navigationsState.showClubsList
  ); // クラブ一覧表示切り替え用のstate
  const [isStaidingView, setIsStandingView] = useState(false); // 順位表表示切り替え用のstate
  const [isGameResultsView, setIsGameResultsView] = useState(
    navigationsState.showGameResults
  ); // 試合結果表示切り替え用のstate
  ...
  return (
    <div>
      ...
      <nav className="button-container">
        <button onClick={switchToClubsList} disabled={isClubsListView}>
          Clubs
        </button>
        <button onClick={switchToStanding} disabled={isStaidingView}>
          Standing
        </button>
        <button onClick={switchToGameResults} disabled={isGameResultsView}>
          Game Results
        </button>
      </nav>
      ...
      <main>
        ...
        {isClubsListView && (
          <>
            <section>
              {/* リーグ名を表示 */}
              {league && <h1>{league.name} Clubs</h1>}{" "}
              {/* リーグ名を表示する要素を追加 */}
              {/* クラブ一覧 */}
              {!clubs || clubs.length === 0 ? (
                ...
                <ul>
                  {clubs.map((club) => (
                    <li key={club.id}>
                      <Link
                        to={`/countries/${countryId}/leagues/${leagueId}/clubs/${club.id}/players`}
                      >
                        {club.name}
                      </Link>
                    </li>
        ...
        {isStaidingView && (
          <>
            {/* リーグ名を表示 */}
            {league && <h1>{league.name} Staning</h1>}
                  ...
                  <tbody>
                    {standing.clubForStandings.map((clubForStanding) => (
                      <tr key={clubForStanding.club.id}>
                        ...
                        <td className="text">
                          <Link
                            to={`/countries/${countryId}/leagues/${leagueId}/clubs/${clubForStanding.club.id}/players`}
                          >
          ...
          <>
            ...
            <nav>
              {selectedSeason && selectedSeason.current === true && (
                <Link
                  to={`/countries/${countryId}/leagues/${leagueId}/register-game-result`}
            ...
            {!seasonGameResult ||
            !seasonGameResult.dayGameResults ||
            seasonGameResult.dayGameResults.length === 0 ? (
              <p>No game result data</p>
            ) : (
              seasonGameResult.dayGameResults.map((dayGameResult) => (
                <section key={dayGameResult.gameDate}>
                    ...
                    <tbody>
                      {dayGameResult.gameResults.map((gameResult) => (
                        <tr key={gameResult.id}>
                          <td className="home">
                            <Link
                              to={`/countries/${countryId}/leagues/${leagueId}/clubs/${gameResult.homeClubId}/players`}
                            >
                          ...
                          <td className="away">
                            <Link
                              to={`/countries/${countryId}/leagues/${leagueId}/clubs/${gameResult.awayClubId}/players`}
                            >
